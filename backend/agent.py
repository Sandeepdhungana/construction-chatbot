...
class AgentOrchestrator:
    ...
    def __init__(self, user_id: Optional[int] = None) -> None:
        ...
        self.system_message = (
            ...
            "2. Payment system data (payments)\n"
            ...
            "   → Review the table structures to understand available columns across ALL tables\n"
            "   → CRITICAL: If multiple tables exist, you MUST search MULTIPLE tables, not just one\n"
            "   → If tables exist, you MUST call Generic_Spreadsheet_Query_Tool:\n"
            "     * First, search without table name to let tool infer (searches all relevant tables)\n"
            "     * Then, if multiple tables are relevant, search EACH table explicitly\n"
            "     * Use 'table=<name>::question=<query>' to target specific tables\n"
            "   → For queries spanning multiple tables, use Multi_Table_Analysis_Tool\n"
            "   → CRITICAL: Don't stop after finding info in one table - check ALL relevant tables\n"
            ...
            "   → For questions about people, search CSV/Excel for names across ALL tables\n"
            "   → Combine results from multiple CSV/Excel files in your answer\n\n"
            "STEP 2: Search Payment system data (ALWAYS check if relevant)\n"
            ...
            "3. Searched relevant payment data (List_Payments)\n"
            ...
            "CSV/EXCEL SEARCH STRATEGY - SEARCH MULTIPLE TABLES:\n"
            "1. ALWAYS start with List_Available_Tables to see:\n"
            "   → What tables exist (there may be MULTIPLE CSV/Excel files)\n"
            "   → What columns are in each table\n"
            "   → Column data types and sample values\n"
            "   → This helps you understand what data is available across ALL files\n\n"
            "2. When querying spreadsheets - SEARCH MULTIPLE TABLES:\n"
            "   → CRITICAL: Don't stop after searching one table - check ALL relevant tables\n"
            "   → If List_Available_Tables shows multiple tables, you MUST search multiple tables\n"
            "   → Use Generic_Spreadsheet_Query_Tool multiple times if needed:\n"
            "     * First, search without specifying a table to let the tool infer\n"
            "     * Then, if multiple tables exist, search each relevant table explicitly\n"
            "     * Use 'table=<name>::question=<query>' format to search specific tables\n"
            "   → The tool will automatically find relevant columns and filter\n"
            "   → For example: 'Show all payments over $5000' - search ALL tables with payment/amount columns\n"
            "   → For example: 'List workers with hourly rate above $25' - search ALL worker-related tables\n"
            "   → Combine results from multiple tables in your answer\n\n"
            "3. For complex multi-table queries - USE Multi_Table_Analysis_Tool:\n"
            "   → When information spans multiple CSV/Excel files, use Multi_Table_Analysis_Tool\n"
            "   → Format: 'tables=<table1,table2,table3>::operation=<description>'\n"
            "   → This tool can join, combine, and analyze data across multiple tables\n"
            "   → Example: 'tables=workers,materials::operation=Find workers assigned to material procurement'\n"
            "   → Example: 'tables=payments,invoices::operation=Calculate total payments per project'\n"
            "   → ALWAYS use this when you need to combine data from multiple sources\n\n"
            "4. MULTI-SOURCE SYNTHESIS:\n"
            "   → After searching multiple CSV/Excel tables, combine the results\n"
            "   → If Table A has partial info and Table B has more details, mention BOTH\n"
            "   → Cite which table(s) each piece of information came from\n"
            "   → Don't assume one table has complete information - check ALL tables\n\n"
            
            "ERP SEARCH STRATEGY:\n"
            "1. For ANY question involving people or entities:\n"
            "   → Use Generic_Spreadsheet_Query_Tool for CSV/Excel data\n"
            ...
            "     STEP 3: Check if question involves payments → Call relevant payment tools:\n"
            ...
            "     * Relevant payment tools (List_Payments)\n"
            ...
            "  → STEP 1: Call List_Available_Tables (may show payments.csv, invoices.csv, transactions.csv)\n"
            "  → STEP 2: Search MULTIPLE CSV tables:\n"
            "     * Generic_Spreadsheet_Query_Tool with 'table=payments::question=payments due soon'\n"
            "     * Generic_Spreadsheet_Query_Tool with 'table=invoices::question=invoices due dates'\n"
            "     * Generic_Spreadsheet_Query_Tool with 'table=transactions::question=upcoming transactions'\n"
            "  → STEP 3: If data spans multiple tables, use Multi_Table_Analysis_Tool:\n"
            "     * 'tables=payments,invoices::operation=Find all payments and invoices due in next 7 days'\n"
            "  → STEP 4: Call Get_Payments_Due_Soon to get ERP payment data\n"
            "  → STEP 5: Call List_Payments with filters for 'pending' status\n"
            "  → STEP 6: Synthesize payment information from ALL CSV tables AND ERP\n\n"
    ...
    def run(self, message: str, session_id: str, user_id: Optional[int] = None) -> str:
        ...
        enhanced_message = (
            ...
            "1. Call List_Available_Tables FIRST to see ALL available CSV/Excel tables\n"
            "2. If multiple CSV/Excel tables exist, you MUST search MULTIPLE tables:\n"
            "   - Call Generic_Spreadsheet_Query_Tool - it will automatically search ALL relevant tables\n"
            "   - If needed, call it multiple times with different table names: 'table=<name>::question=<query>'\n"
            "   - For complex multi-table queries, use Multi_Table_Analysis_Tool\n"
            "   - DO NOT stop after finding info in one table - check ALL relevant tables\n"
            "3. Call relevant payment tools (List_Payments) if question involves payments\n"
            "4. Call General_Document_Retriever to search documents\n"
            "5. Call PDF_Document_Retriever, DOCX_Document_Retriever if needed\n"
            "6. Combine information from ALL sources searched\n"
            "7. Only respond after searching ALL sources. Do not skip CSV/Excel searches.\n"
            "8. If no relevant information is found after searching ALL sources, say 'I don't have that information in the available documents and data.'\n\n"
            "CRITICAL: Generic_Spreadsheet_Query_Tool now automatically searches ALL relevant tables when no specific table is specified. "
            "Use it once with your question and it will search across multiple tables automatically."
        )
